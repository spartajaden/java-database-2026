...
- DDL 명령어 - [쿼리](./day05/5.DDL.sql)
    ...
    - `TRUNCATE` - 객체 내 데이터 모두 삭제. 대부분 테이블에서 진행
        ...
        ```sql
        -- DELETE FROM 테이블명 과 동일. 단, 트랜잭션이 발생하지 않아 무조건 복구불가
        -- 테이블 생성 이후 상태로 됨.
        TRUNCATE TABLE 테이블명
...
- 데이터 사전 - 일반 테이블 외에 DB를 운영하는 데 필요한 특수한 테이블
    - USER_XXXX - 현재 DB에 접속된 사용자가 소유한 객체 정보
    - ALL_XXXX - 사용허가를 받은 객체 정보
    - DBA_XXXX - DB 관리를 위한 정보(SYSTEM, SYS 사용자만 접근가능)
    - V$_XXXX - DB 성능관련 정보

- 인덱스 
    - `Full Table Scan` - 모든 테이블 데이터를 처음부터 끝까지 읽는 기능
    ...
    - 대용량 데이터 (대략 몇천만권)에서 속도개선을 위해서 인덱스 사용
...
- 뷰
    ...
    - `주의!` 보기위한 객체지만 한 테이블의 SELECT * 일 뷰 경우 INSERT 가능
...
- 데이터 무결성
    ...
    - 영역 무결성(적절한 형식이 데이터나 NULL 불가), 개체 무결성(pk개념), 참조 무결성(FK개념)

- CASCADE 계단식처리
    - 부모 테이블의 PK 컬럼 데이터를 지우면 자식 테이블의 FK에 참조중인 레코드를 전부 지우는 기능
...
- 논리설계 이전 기본키/외래키, 일반속성 정립
    ...
    - 의사 - `의사번호`, 이름, 진료과목, 주소, 전화번호, ~~주민번호~~, 직급, _과번호_, ~~의사약력~~
    - 간호사 - `간호사번호`, 이름, 진료과목, 주소, 전화번호, ~~주민번호~~, 직급, _과번호_
    - 환자 - `환자번호`, 이름, 주민번호, 전화번호, 주소, 키, 몸무게, 혈액형, 성별, 진료과, **진료과**, **처치일**, **처치내용**
    - 입원 - `입원번호`, 입원일, 퇴원일, 병실
...
3. 관계(없거나 한개 또는 여러개) 클릭
    ...
    - 비식별관계는 단순 FK - NOT NULL 또는 NULL 모두 허용. 단, NOT NULL일 경우 데이터 처리시 어려움 발생
...
- 300만권 더미테이터 생성 - [쿼리](./day10/2.300만건데이터.sql)
